...
public class PlayerRowMapper implements JdbcTemplate.RowMapper<List<Player>> {
    public List<Player> mapRow(ResultSet rs) throws SQLException {
        ...
        while(rs.next()){
            ...
            Player player = new Player(id, name, heightCM, countryName, clubName, overall, Arrays.asList(positions.split(",")));
            players.add(player);
        }
        return players;
    }
